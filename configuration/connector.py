...
def update_fivetran(conf):
    """Update Fivetran with the latest configuration."""
    conf["test"] = "foo"
    log.fine(conf)
    ...
    fivetran_headers = {
        ...
        "Authorization": f"Basic {conf['fivetranAPIKey']}",
        "Content-Type": "application/json"
    }

    config_patch = {"config": {"secrets_list": config_list}}

    log.info(f"Payload sent to Fivetran: {json.dumps(config_patch, indent=2)}")  # Debugging

    response = rq.patch(f"https://api.fivetran.com/v1/connectors/{conf['connectorId']}",
                        json=config_patch, headers=fivetran_headers)
